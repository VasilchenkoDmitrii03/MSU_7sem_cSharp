    ...
    public class RouteOptimizer
    {
        ...
        public Route Calculate(int populationSize = 100, int generationsCount = 5)
        {
            ...
            for (int i = 0; i < generationsCount; i++)
            {
                ...
                if (i % 3 == 0)
                {
                    bestRoute = currentPopulation[scores.IndexOf(scores.Max())];
                    double bestLength = bestRoute.CalculateDistance(DistanceMatrix);
                    Console.WriteLine($"Generation {i}: best route length = {bestLength} on route {bestRoute.ToString()}");
                }
            }
